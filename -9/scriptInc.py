...
def fail_report() :
    ...
    scriptSys.GUI['line1'] = "Analysis Finished"
    scriptSys.GUI['line2'] = "Batery Fail VOLTAGE :"+ str(scriptSys.VOLTAGE) + 'V'
    scriptSys.GUI['bgcolor'] = '"120,0,0"'
    scriptSys.GUI['extra_info'] = " Z1="+scriptSys.EVAL['int_z1']+" Z2="+scriptSys.EVAL['int_z2']
    scriptSys.ini_Update()
    scriptSys.copy_report()
    sys.exit()
...
FailVoltage1= 100
#Setup
tTest12_s1  =   30  #tiempo de descarga suave
tTest22_s1  =   60  #tiempo de descarga fuerte
tTest32_s1  =   60  #tiempo de recuperacion
tTest42_s1  =   20  #tiempo de chequeo e incio de sig etapa
tTest52_s1  =   20
# tMargin =   5   #margen de tiempo por no ser 10s exactos
tTestA2_s1  =   tTest12_s1
tTestB2_s1  =   tTest12_s1 + tTest22_s1
tTestC2_s1  =   tTest12_s1 + tTest22_s1 + tTest32_s1
tTestD2_s1  =   tTest12_s1 + tTest22_s1 + tTest32_s1 + tTest42_s1
#
def stress_state1() :
    if scriptSys.GENERAL['mode'] != 'STRESS1' : #si es llamado por primera vez
        scriptSys.GENERAL['mode'] = 'STRESS1'
        scriptSys.TIME_INIT = scriptSys.TIME
        scriptSys.GENERAL['line_m'] = scriptSys.VOLTAGE
        print "DISCHARGE,1.0"
        return

    actual_time = (scriptSys.TIME - scriptSys.TIME_INIT)
    # if  actual_time >= tTestD2_s1 :
    #     # deja reposar y chequea q no caiga la tension
    #     # final_report()
    #     stress_test()
    #     return

    # if  actual_time >= (tTestC2_s1 - tMargin)  and  actual_time < (tTestC2_s1 + tMargin) :
    #     # print "DISCHARGE,1.0"  Descarga fuerte
    #     scriptSys.import_data()
    #     t = scriptSys.TIME_INIT + tTestB2 + 2 #delay en el inicio de la descarga
    #     var = scriptSys.get_data('VOLTAGE', range( t - 5 , t - 5 + voltageAverage))
    ...
    if  actual_time >= (tTestB2_s1 - tMargin)  and  actual_time < (tTestB2_s1 + tMargin) :
        # deja reposar y chequea q no caiga la tension
        # stress_test()
        # print "DISCHARGE,1.5"
        if (scriptSys.GENERAL['line_m'] - scriptSys.VOLTAGE) <=  FailVoltage1:
            fail_report()
        else:
            zmeasure_state()
        return

    if  actual_time >= (tTestA2_s1 - tMargin)  and  actual_time < (tTestA2_s1 + tMargin) :
        # print "DISCHARGE,0.2"  Descarga suave
        # scriptSys.import_data()
        # t = scriptSys.TIME_INIT + 2 #delay en el inicio de la descarga
        # var = scriptSys.get_data('VOLTAGE', range( t - 5, t- 5 + voltageAverage))
...
FailVoltage2= 100
#Setup
tTest12_s2  =   30  #tiempo de descarga suave
tTest22_s2  =   60  #tiempo de descarga fuerte
tTest32_s2  =   60  #tiempo de recuperacion
tTest42_s2  =   20  #tiempo de chequeo e incio de sig etapa
tTest52_s2  =   20
# tMargin =   5   #margen de tiempo por no ser 10s exactos
tTestA2_s2  =   tTest12_s2
tTestB2_s2  =   tTest12_s2 + tTest22_s2
tTestC2_s2  =   tTest12_s2 + tTest22_s2 + tTest32_s2
tTestD2_s2  =   tTest12_s2 + tTest22_s2 + tTest32_s2 + tTest42_s2
#
def stress_state2() :
    if scriptSys.GENERAL['mode'] != 'STRESS2' : #si es llamado por primera vez
        scriptSys.GENERAL['mode'] = 'STRESS2'
        scriptSys.TIME_INIT = scriptSys.TIME
        scriptSys.GENERAL['line_m'] = scriptSys.VOLTAGE
        print "DISCHARGE,1.0"
        return
    ...
    actual_time = (scriptSys.TIME - scriptSys.TIME_INIT)
    # if  actual_time >= tTestD2_s2 :
    #     # deja reposar y chequea q no caiga la tension
    #     # final_report()
    #     stress_test()
    #     return

    # if  actual_time >= (tTestC2_s2 - tMargin)  and  actual_time < (tTestC2_s2 + tMargin) :
    #     # print "DISCHARGE,1.0"  Descarga fuerte
    #     scriptSys.import_data()
    #     t = scriptSys.TIME_INIT + tTestB2 + 2 #delay en el inicio de la descarga
    #     var = scriptSys.get_data('VOLTAGE', range( t - 5 , t - 5 + voltageAverage))
    ...
    if  actual_time >= (tTestB2_s2 - tMargin)  and  actual_time < (tTestB2_s2 + tMargin) :
        # deja reposar y chequea q no caiga la tension
        # stress_test()
        # print "DISCHARGE,1.5"
        if (scriptSys.GENERAL['line_m'] - scriptSys.VOLTAGE) <=  FailVoltage1:
    ...
    if  actual_time >= (tTestA2_s2 - tMargin)  and  actual_time < (tTestA2_s2 + tMargin) :
        # print "DISCHARGE,0.2"  Descarga suave
        # scriptSys.import_data()
        # t = scriptSys.TIME_INIT + 2 #delay en el inicio de la descarga
        # var = scriptSys.get_data('VOLTAGE', range( t - 5, t- 5 + voltageAverage))
